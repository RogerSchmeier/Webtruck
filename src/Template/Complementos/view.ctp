<div class="actions columns large-2 medium-3">
    <h3><?= __('Ações') ?></h3>
    <ul class="side-nav">
        <li><?= $this->Html->link(__('Editar'), ['action' => 'edit', $complemento->id], ['class'=>'btn btn-default btn-xs']) ?> </li>
        <li><?= $this->Form->postLink(__('Deletar'), ['action' => 'delete', $complemento->id],['class'=>'btn btn-danger btn-xs'], ['confirm' => __('Are you sure you want to delete # {0}?', $complemento->id)]) ?> </li>
        </ul>
</div>
<div class="complementos view large-10 medium-9 columns">
    <h2><?= h($complemento->id) ?></h2>
    <div class="row">
        <div class="large-5 columns strings">
            <h6 class="subheader"><?= __('Nome') ?></h6>
            <p><?= h($complemento->nome) ?></p>
        </div>
        <div class="large-2 columns numbers end">
            <h6 class="subheader"><?= __('Id') ?></h6>
            <p><?= $this->Number->format($complemento->id) ?></p>
        </div>
    </div>
    <div class="row texts">
        <div class="columns large-9">
            <h6 class="subheader"><?= __('Obs') ?></h6>
            <?= $this->Text->autoParagraph(h($complemento->obs)); ?>
...
<div class="related row">
    ...
    <?php if (!empty($complemento->abastecimentos)): ?>
    <table cellpadding="0" cellspacing="0">
        ...
        <?php foreach ($complemento->abastecimentos as $abastecimentos): ?>
        <tr>
